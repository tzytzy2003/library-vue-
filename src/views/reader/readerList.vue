<template>
  <el-main>
    ...
    <el-table :height="tableHeight" :data="tableData" border stripe>
      ...
      <el-table-column prop="username" label="社区号码" width="100" />
      <el-table-column prop="className" label="社区" width="120" />
      ...
      <el-table-column prop="checkStatus" label="审核状态">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.checkStatus == '0'" type="danger">未审核
          </el-tag>
      ...
      <el-table-column label="操作" fixed="right" width="550">
        <template slot-scope="scope">
          <el-button v-permission="['sys:reader:edit']" type="primary" @click="editBtn(scope.row)"> 编辑
          </el-button>
          ...
          <el-button v-permission="['sys:reader:delete']" type="danger" @click="deleteBtn(scope.row)"> 删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <sys-dialog :title="dialog.title" :width="dialog.width" :height="dialog.height" :visible="dialog.visible"
      ...
      <div slot="content">
        <el-form ref="addRef" :model="addModel" :rules="rules" label-width="80px" style="margin-right: 30px">
          ...
          <el-row>
            <el-col :span="12" :offset="0">
              <el-form-item prop="username" label="社区号码">
                <el-input v-model="addModel.username" />
              </el-form-item>
            </el-col>
            <el-col :span="12" :offset="0">
              <el-form-item prop="className" label="社区">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        className: [{ required: true, message: '请填写社区', trigger: 'blur' }]
      },
      // 表单属性
      addModel: {
        type: '',
  ...
  methods: {
    ...
    async applyBtn(row) {
      const confirm = await this.$myconfirm('确定通过审核吗?')
      if (confirm) {
        console.log(row)

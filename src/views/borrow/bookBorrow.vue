<template>
  <el-main>
    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <el-form :model="searchParm" label-width="80px" :inline="true">
          <el-form-item label="社区号码">
            <el-input v-model="searchParm.username" />
          </el-form-item>
          <el-form-item>
            <el-button @click="getByUserName"> 查询
            </el-button>
            <el-button style="color: #ff7670" @click="resetBtn"> 重置
            </el-button>
          </el-form-item>
          <el-form-item label="还书时间">
            <el-date-picker v-model="returnTime" type="date" placeholder="选择还书时间" />
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="borrowBtn"> 借书
            </el-button>
          </el-form-item>
        </el-form>
      </div>
    ...
    <elt-transfer ref="eltTransfer" v-model="tableData" :show-query="true" :show-pagination="true"
      ...
      :button-texts="[' 添加', ' 删除']" :query-texts="[' 查询', ' 查询']" :table-row-key="(row) => row.bookId">
      <!-- 可以使用插槽获取到列信息和行信息，从而进行数据的处理 -->
      <template v-slot:leftCondition="{ }">
        <el-form-item label="图书名称">
          <el-input v-model="listParm.bookName" placeholder="图书名称" />
        </el-form-item>
